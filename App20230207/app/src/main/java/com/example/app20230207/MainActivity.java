package com.example.app20230207;
...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    // onCreate란 액티비티 클래스의 생명주기 메서드 중, 초기화를 담당하는 메서드이다
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        // 개발자는 안드로이드 화면을 구성할 UI컴포넌트를 직접 자바 코드로 작성할 수 있지만,
        // 개발의 효율성이 떨어지므로
        // 특별한 이유가 아니라면 XML에서의 작업이 효율성이 높다
        // 아래의 setContentView() 메서드는 개발자가 자바 코드 대신 태그 형식으로 작성한 디자인 코드를 해석하여
        // 실제 안드로이드의 컴포넌트로 메모리에 올려진다(내부적으로 파싱작업)
    ...
    // 이 때 매개변수로 넘어오는 View는 이벤트를 일으킨 View인 버튼이다
    @Override
    public void onClick(View view) {
        String msg = "왜이래";
        Log.d(TAG, msg);
